...
import { loadZones, quoteShipping } from "../../src/engine/domain/shipping";
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    const schema = TOOL_SCHEMAS["calculate_shipping"];
    ...
    const output = quoteShipping(catalog, zones, body as any);
    ...
    const local = (() => {
      try {
        return quoteShipping(loadCatalog(), loadZones(), { items: [], zone: 1, service: "ground" } as any);
      } catch {
        return null;
      }
    })();
