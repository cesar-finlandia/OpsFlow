...
function planDeterministicInline(goal: string, catalog: ReturnType<typeof loadCatalog>): ToolPlan {
  try {
    ...
    let maxPriceCents: number | null = null; const pm = g.match(/(?:under|below|lower than|less than|cheaper than|<)\s*\$?\s*(\d+(?:\.\d{1,2})?)/); if (pm && pm[1]) { const v = parseFloat(pm[1]); if (!Number.isNaN(v)) maxPriceCents = Math.round(v*100); }
