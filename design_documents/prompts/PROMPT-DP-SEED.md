# PROMPT — Author design plan `DP-SEED` (Synthetic fixtures, mock script and golden-cache seeds)
...
hackathon entry. Produce exactly one file:
    ...
    design_documents/design_plans/DP-SEED.md
...
# PART FOUR — what `DP-SEED` must own

`DP-SEED` owns every fixture the app and the rehearsal depend on: the 200-variant synthetic
catalog, the five-zone shipping table, the manual baseline, the mock envelope script, the
demodrive click script, and the golden-cache seeds. It ships **data**, not runtime code: the only
code it owns is the generator script that produces those files, which never runs in production.

## 6.1 Required outputs (contract-table rows 28–30)

| File | Shape | Consumers |
|---|---|---|
| `data/catalog.json` | `Catalog` (§5A) — 60 products, **200 variants total**, every record `synthetic: true` | `DP-DOM`, `DP-SRV`, `DP-UI` |
| `data/zones.json` | `ZoneTable` (contract row 14) — zones `1`–`5`, `base_cents`, `per_100g_cents`, `service_multiplier` for `ground/expedited/overnight`, and the surcharge list `OVERSIZE`, `REMOTE`, `EXPEDITED_FUEL` | `DP-DOM`, `DP-SRV` |
| `data/baseline.json` | `{ baseline_minutes: 25, baseline_clicks: 120, source: "winning_project_plan.md §Problem Framing", synthetic: true }` | `DP-UI` (`SavingsMeter`) |
| `demo/mock-script.json` | chassis `contracts/mock-script.schema.json` — the full golden batch as envelopes | `DP-DEV` (`chassis mock`), `DP-PITCH` |
| `demo/click-script.json` | chassis demodrive click script for the same batch | `DP-PITCH` |
| `.cache/golden/*.json` | chassis golden-cache entries: one per tool output plus one planner response | `DP-CORE` (`guarded`), `DP-DEV` |
| `scripts/seed.ts` | the generator (dev-only) | operator |

## 6.2 Algorithms your plan MUST spell out step by step

1. **Catalog generation** — deterministic, seeded, reproducible:
   1) fix `SEED = 20260901` and use a small LCG written out in full (`next = (a*seed + c) % m`), so
      two runs produce byte-identical files;
   2) 12 categories × 5 products = 60 products; product ids `OPS-1001`..`OPS-1060`;
   3) each product gets 3–4 variants drawn from sizes `["XS","S","M","L","XL"]` and colours
      `["Blue","Black","Sand","Olive","Crimson"]` — list the exact allocation rule — totalling
      **exactly 200** variants (state how the remainder is distributed and assert the count);
   4) `sku = "<product_id>-<COLOR3>-<SIZE>"`, e.g. `OPS-1042-BLU-M`;
   5) `price_cents` uniform in 600..4800 rounded to the nearest 50;
   6) `stock` distribution: 15 % zero, 25 % in 1..5, 60 % in 6..80 — this guarantees the demo goal
      "low-stock" returns a non-trivial set;
   7) `low_stock_threshold = 5` for every variant (one definition, matching `isLowStock`);
   8) `weight_g` uniform 120..2400;
   9) every object carries `synthetic: true`; the file carries `version`, `generated_at`,
      `synthetic: true`.
   **Guarantee to state explicitly and assert in a test:** the canonical demo goal *"hold all
   low-stock blue variants under $12 shipping to zone 4"* must return **between 4 and 12** matches.
   Write the assertion.
2. **Optional LLM-assisted naming** — product titles may come from the chassis `data` module:
   `setProviderCall(...)` then `generateRecords({ domain: "apparel ops catalog", shape: { kind:
   "records", schema, count: 60 }, watermark: true })`. If the call degrades
   (`isDegradedResult`), fall back to a checked-in list of 60 titles — write that list into the
   plan. The generated file must be byte-stable afterwards, so the LLM path runs **once** and its
   output is committed; state this.
3. **Zone table** — write all five zones' numbers literally, plus each surcharge's condition and
   amount, matching `DP-DOM` §quoteShipping step 7 exactly. Two sources of truth would produce a UI
   that disagrees with the tool, so state that `DP-DOM` reads these numbers and never hardcodes them.
4. **Mock script** — one envelope sequence covering the whole golden batch: `agent.plan`
   started/done, then for each of the five tools `started` then `done` with a realistic payload,
   plus one `session.confirm` pair with `granted: true`. Give per-envelope `delay_ms` values summing
   to ~40 s so a rehearsal runs at demo pace. Validate against
   `contracts/mock-script.schema.json`.
5. **Golden-cache seeding** — 1) run the batch once against the real code; 2) for each tool call
   compute the key with `goldenCache.deriveKey({ provider: "opsflow", model: <tool name>, prompt:
   canonicalJson(args) })`; 3) `goldenCache.put(key, outcome, { seeded: true })`; 4) commit the
   resulting files so a clean clone can replay offline. Give the exact command.

## 6.3 Resiliency + honesty requirements

Every fixture carries `synthetic: true` and the chassis watermark header where the format allows
(`NFR-04`). No real merchant, customer, address or order data exists anywhere. The UI's "synthetic
data" badge reads from `data/catalog.json`'s own flag, so the badge cannot drift from the data.

## 6.4 Work units

| WU | Deliverable | Verify command | Expected output |
|---|---|---|---|
| `W1` | `scripts/seed.ts` + `data/catalog.json` | `npx vite-node scripts/seed.ts && node -e "const c=require('./data/catalog.json');console.log(c.products.length, c.products.reduce((n,p)=>n+p.variants.length,0))"` | `60 200` |
| `W2` | determinism check | run `seed.ts` twice, `git diff --stat data/catalog.json` | no changes |
| `W3` | `data/zones.json` + `data/baseline.json` | `node -e "const z=require('./data/zones.json');console.log(Object.keys(z.zones).join(','), z.surcharges.length)"` | `1,2,3,4,5 3` |
| `W4` | demo-goal guarantee test | `npm run test -- tests/seed/demo-goal` | 4 ≤ matches ≤ 12 |
| `W5` | `demo/mock-script.json` + `demo/click-script.json` | `chassis mock --script demo/mock-script.json` | 14 envelopes published, exit 0 |
| `W6` | `.cache/golden/` seeds committed | `RES_FORCED_DEGRADED=1 npm run demo:offline` | the five-step batch completes from cache |

---

# PART FIVE — required structure of the document you write

Write **one** file: `design_documents/design_plans/DP-SEED.md`.

```
# DP-SEED — Synthetic fixtures, mock script and golden-cache seeds

## 1. Purpose & Scope
## 2. Requirements Traceability   (§3A block verbatim; DP-SEED enables FR-15, FR-16, FR-20, NFR-04; inherits the rest)
## 3. Architecture                (build-time only; what the app imports at runtime is JSON, nothing else)
## 4. Interfaces                  (rows 28–30 with full file shapes; the Catalog and ZoneTable types from §5A)
## 5. Algorithms                  (§6.2 items 1–5 as numbered steps, with the LCG and the 60 fallback titles written out)
## 6. Configuration               (SEED constant, size/colour sets, stock distribution, threshold)
## 7. Resiliency & Honesty        (§6.3; degraded LLM path falls back to the checked-in title list)
## 8. File Layout & Module Boundaries
## 9. Work Units                  (W1..W6 with Verify: lines and commit messages)
## 10. Testing Strategy           (determinism, counts, the demo-goal guarantee, schema validation of the mock script)
## 11. Dependencies & Dependents   (chassis data module + row 7 goldenCache; consumed by DP-DOM, DP-SRV, DP-UI, DP-DEV, DP-PITCH)
## 12. Non-Goals Affirmation       (no real data, no runtime generation, no PII, no scraping a real store, no chassis edits)
## Appendix A. Worked example      (the first three catalog records, the zone-4 row, and the first four mock envelopes, in full JSON)
```

## Finish

Write that one file and stop. Do not implement any code and do not generate the data yourself.
