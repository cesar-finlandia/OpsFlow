# PROMPT — Author design plan `DP-UI` (Console UI: three screens, inspector, timeline, dialogs, declarative form)
...
hackathon entry. Produce exactly one file:
    ...
    design_documents/design_plans/DP-UI.md
...
# PART FOUR — what `DP-UI` must own

`DP-UI` owns `src/main.tsx` and everything under `src/ui/**`: three screens, the Tool Inspector,
the co-execution timeline, the confirmation dialog, the two banners, the savings meter, and the
**declarative-API annotated shipping form** (`MAN-02`). It re-implements no domain rule, never
calls `registerTool`, and never fetches except through `apiClient`.

## 6.1 Required public surface

Contract-table row 27 is yours; everything else in `src/ui/**` is private to `DP-UI` — say so.

```ts
// src/main.tsx  — order matters and is frozen:
//   1) setTheme("operator")                      (chassis platform/ui)
//   2) setConfirmationHandler(uiConfirm)         (DP-TOOLS bridge, before any tool can run)
//   3) await registerAllTools()                  (DP-TOOLS, BEFORE first render — FR-01)
//   4) createRoot(...).render(<App probe={...}/>)

// src/ui/state/session.ts
export function useSession(): {
  envelopes: EventEnvelope[]; degraded: boolean; holds: Hold[];
  meter: SavingsMeter; lastQuote: ShippingQuote | null; resultSkus: string[];
};
export function sessionResultSkus(): string[];    // read by DP-TOOLS for filter_variants context (FR-03)
export function lastQuote(): ShippingQuote | null; // read by DP-TOOLS for hold_order
```

`sessionResultSkus()` and `lastQuote()` are the two functions `DP-TOOLS` imports from `DP-UI`.
They must be plain module functions backed by a module-level store — **not** React hooks — so a
tool call outside React can read them.

## 6.2 Components and their exact props (define all of these)

| Component | File | Props | Behaviour |
|---|---|---|---|
| `App` | `src/ui/App.tsx` | `{ probe: ReturnType<typeof probeWebMcp> }` | Tab shell over the three screens; renders `WebMcpBanner`, `DegradedBanner`, `SavingsMeter`, `CoExecutionTimeline`, `ToolInspector`, `ConfirmDialog` |
| `BatchScreen` | `screens/BatchScreen.tsx` | none | Goal input + "Run with agent" button → `orchestrator.run(goal)`; results table of `VariantMatch` with low-stock chips; row checkboxes feed `lineItems` |
| `ShippingScreen` | `screens/ShippingScreen.tsx` | none | Quote card with a collapsible **explain** list; the declarative annotated `<form>` (`MAN-02`) |
| `HoldsScreen` | `screens/HoldsScreen.tsx` | none | `holdsStore.list()` with live TTL countdown, Release and Confirm buttons, committed banner |
| `ToolInspector` | `components/ToolInspector.tsx` | `{ open: boolean }` | Reads `document.modelContext.getTools()` **live**; falls back to `TOOL_SCHEMAS` with a "not registered" note; shows name, description, annotations and pretty-printed schema (`FR-10`) |
| `CoExecutionTimeline` | `components/CoExecutionTimeline.tsx` | `{ envelopes: EventEnvelope[] }` | One row per envelope: step id, status, elapsed ms, the exact validated args, degraded chip. Built on the chassis `StepStatusIndicator` |
| `ConfirmDialog` | `components/ConfirmDialog.tsx` | `{ request, onResolve }` | Focus-trapped modal; Escape and the backdrop resolve `false`; shows the summary and a pretty-printed args block; the only path to `true` is the Confirm button |
| `DegradedBanner` | `components/DegradedBanner.tsx` | `{ degraded: boolean; reason?: string }` | Full-width; text per ladder rung |
| `WebMcpBanner` | `components/WebMcpBanner.tsx` | `{ probe }` | Shown only when `!probe.available`; names both enablement paths verbatim (ChatGPT desktop in-app browser; Chrome 149+ with `chrome://flags/#enable-webmcp-testing`, then restart) and states the in-page console still works (`FR-17`) |
| `SavingsMeter` | `components/SavingsMeter.tsx` | `{ meter: SavingsMeter }` | "3 tool calls · 1 confirmation · 41 s — manual baseline 25 min / 120 clicks" (`FR-15`) |

## 6.3 Algorithms your plan MUST spell out step by step

1. **`uiConfirm(req)`** — 1) push `req` into the dialog state; 2) return a promise resolved by the
   dialog's `onResolve`; 3) on unmount resolve `false`; 4) only one dialog at a time — queue further
   requests. Write the code.
2. **Session store** — 1) subscribe to `useEventStream()` (chassis) for envelopes; 2) on a
   `tool.search_inventory` or `tool.filter_variants` `done` envelope whose outcome is `ok`, replace
   `resultSkus` with `matches.map(m => m.sku)`; 3) on a `tool.calculate_shipping` `done` `ok`,
   store `lastQuote`; 4) subscribe to `holdsStore.subscribe` for holds; 5) meter: increment
   `tool_calls` on every `tool.*` `started`, `confirmations` on every `session.confirm` `done` with
   `granted: true`, and track `elapsed_ms` from the first `started` of the current trace;
   6) `degraded` is true when any envelope in the current trace has `degraded: true`.
3. **The declarative form (`MAN-02`)** — write the JSX literally, including the annotation
   attributes that mark it as a WebMCP declarative tool (`toolname`, per-field `tooldescription`
   annotations on the `zone`, `service` and `skus` inputs) and the `onSubmit` handler that calls
   `executeToolCompat("calculate_shipping", …)`. State that this is the `MAN-02` proof and that
   `rg -n "toolname" src/` must return exactly this one hit.
4. **First-paint order** — the four `main.tsx` steps above, in that exact order, with a comment on
   why registration precedes render (`FR-01`, and `NFR-10`'s < 1.5 s budget).
5. **Empty, loading and error states** — specify, per screen, the literal text shown when: no goal
   has run; a run is in progress; a run returned zero matches; a hold expired while on screen.

## 6.4 Resiliency + accessibility requirements

Every screen renders with `data/catalog.json` alone — no screen depends on a successful network
call. The dialog is focus-trapped, restores focus on close, and is reachable by keyboard only. The
"synthetic data" badge is always visible (`NFR-04`). No `<iframe>` appears anywhere (`NFR-12`).

## 6.5 Work units

| WU | Deliverable | Verify command | Expected output |
|---|---|---|---|
| `W1` | `session.ts` + `main.tsx` boot order | `npm run dev` then browser console `window.__opsflowBootOrder` | `["theme","confirm","register","render"]` |
| `W2` | `App.tsx` shell + both banners | `npm run test -- tests/ui/banners` | passes |
| `W3` | `BatchScreen` + results table | `npm run test -- tests/ui/batch` | passes |
| `W4` | `ShippingScreen` incl. the annotated form | `rg -n "toolname" src/` | exactly one hit |
| `W5` | `HoldsScreen` + `ConfirmDialog` (a11y) | `npm run test:ui` | focus-trap and Escape assertions pass |
| `W6` | `ToolInspector` + `CoExecutionTimeline` + `SavingsMeter` | `npm run test -- tests/ui/inspector` | inspector lists five tools from a faked `getTools()` |

---

# PART FIVE — required structure of the document you write

Write **one** file: `design_documents/design_plans/DP-UI.md`.

```
# DP-UI — Console UI: three screens, inspector, timeline, dialogs, declarative form

## 1. Purpose & Scope
## 2. Requirements Traceability   (§3A block verbatim; DP-UI OWNS MAN-02; enables FR-10, FR-13..FR-15, FR-17, FR-18, NFR-09, NFR-10, NFR-12)
## 3. Architecture                (component tree, the module-level session store, why it is not a hook)
## 4. Interfaces                  (row 27 verbatim + the §6.2 prop tables + the two functions DP-TOOLS imports)
## 5. Algorithms                  (§6.3 items 1–5, with uiConfirm and the annotated form written literally)
## 6. Configuration               (theme "operator"; text of every banner and empty state, verbatim)
## 7. Resiliency & Accessibility  (§6.4 + ladder rungs 4 and 5 as rendered)
## 8. File Layout & Module Boundaries
## 9. Work Units                  (W1..W6 with Verify: lines and commit messages)
## 10. Testing Strategy           (fake document.modelContext; render tests; a11y assertions)
## 11. Dependencies & Dependents   (imports rows 1–26 + chassis platform/ui; consumed by nobody — DP-UI is a leaf)
## 12. Non-Goals Affirmation       (no domain rules, no registerTool call, no direct fetch, no fourth screen, no iframe, no chassis edits)
## Appendix A. Worked example      (the three-screen golden path with the exact on-screen text at each step, matching the video script beats)
```
