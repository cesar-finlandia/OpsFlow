# PROMPT — Author design plan `DP-SRV` (Serverless API and typed client)
...
hackathon entry. Produce exactly one file:
    ...
    design_documents/design_plans/DP-SRV.md
...
# PART FOUR — what `DP-SRV` must own

`DP-SRV` owns the five Vercel serverless routes in `<entry>/api/**` and the typed browser client
`src/engine/apiClient.ts`. Every handler is a **thin wrapper** over `DP-DOM`: it parses, validates,
calls the domain function, and serializes. It never contains a business rule. It owns the only
file where `GEMINI_API_KEY` is read (`api/agent/plan.ts`) — but the prompt building, response
parsing and deterministic fallback inside that file are **imported from `DP-AGENT`**, never
re-written here.

## 6.1 Required public surface

Contract-table row 17 is yours, plus the five routes in §5C.
...
export function apiBase(): string;    // "" in production (same origin); import.meta.env.VITE_API_BASE in dev

// api/health.ts, api/inventory/search.ts, api/inventory/filter.ts,
// api/shipping/quote.ts, api/agent/plan.ts
export default function handler(req: VercelRequest, res: VercelResponse): Promise<void>;
```

Every handler shares one private helper module `api/_shared.ts` (private to `DP-SRV`; state that
no other module may import it) exporting `readJson(req)`, `sendJson(res, status, body)`,
`badRequest(res, message)`, and `withCors(res)` — same-origin only, so CORS sets nothing beyond
`Vary: Origin`.

## 6.2 Algorithms your plan MUST spell out step by step

1. **Handler skeleton (identical for all four data routes)** — 1) if `req.method !== "POST"` reply
   405 `{ ok: false, error: { code: "INVALID_INPUT", message: "POST required" } }`; 2) `readJson`
   with a 32 KB body cap; 3) validate against `TOOL_SCHEMAS[<tool>]` using the chassis
   `validate(value, schema)` from `src/resilience`; on errors reply 400 with
   `makeToolError("INVALID_INPUT", <first error message>, { errors })`; 4) call the `DP-DOM`
   function with `loadCatalog()` / `loadZones()`; 5) reply 200 with `ok(<output>)`; 6) never throw —
   wrap the body in try/catch and reply 200 with a `DEGRADED` outcome carrying the local result if
   one exists, else 500 with a typed error. Write this skeleton once, literally, and say the four
   routes differ only in the schema and the domain call.
2. **`GET /api/health`** — returns `HealthResponse` with: `mode: "live"` unless
   `process.env.RES_FORCED_DEGRADED === "1"`; `origin_isolated: true` (it is a server-side echo of
   the configured headers — state that the authoritative runtime check is `probeWebMcp()` in the
   browser); `planner: process.env.GEMINI_API_KEY ? "gemini-2.5-flash" : "deterministic"`;
   `catalog` counts computed from `loadCatalog()`. Never non-200.
3. **`POST /api/agent/plan`** — 1) read `{ goal, context }`, truncate `goal` to 400 chars;
   2) if `!process.env.GEMINI_API_KEY` → return `planDeterministic(goal, loadCatalog())` with
   `planner: "deterministic", degraded: false`; 3) otherwise build the prompt with
   `buildPlannerPrompt(goal, TOOL_SCHEMAS, context ?? {})` (imported from `DP-AGENT`); 4) call
   Gemini 2.5 Flash **inside** `guarded(...)` from `DP-CORE` (`NFR-06`) with
   `responseMimeType: "application/json"`; 5) `recordUsage({ role: "planner", model:
   "gemini-2.5-flash", prompt_tokens, completion_tokens, ts })`; 6) parse the JSON; validate every
   step against `TOOL_SCHEMAS[step.tool]`; drop invalid steps and note them; 7) if the result is a
   `DegradedResult`, if the parse fails, if zero valid steps survive, or if `overBudget()` is true →
   return `planDeterministic(...)` with `degraded: true`; 8) always reply 200. Write the exact
   request body shape sent to the Gemini endpoint, including the model id and the URL.
4. **`apiClient` fallback rule** — for `search`, `filter` and `quote`: 1) start a `fetch` with
   `AbortController` and `config.tools.network_timeout_ms` (900 ms); 2) on 200 return the parsed
   outcome; 3) on timeout, network error or non-200, call the corresponding `DP-DOM` function
   **locally in the browser** with `loadCatalog()`/`loadZones()`, and return
   `ok(<output>, /* degraded */ true)`; 4) never throw. `plan` falls back to
   `planDeterministic(goal, loadCatalog())`. `health` falls back to a synthesized
   `{ ok: false, mode: "degraded", ... }`. Show the code once and state that the three data methods
   are structurally identical.
5. **No secret in the bundle** — state literally: `GEMINI_API_KEY` is read only in
   `api/agent/plan.ts` via `process.env`; it is never referenced in `src/**`; it is configured with
   `npx vercel env add GEMINI_API_KEY production`; `.env*` is gitignored (`NFR-01`).

## 6.3 Resiliency section requirements

Every route replies 200 or a typed 4xx; nothing 500s on a well-formed request. The client always
degrades to local domain logic rather than surfacing an error. Cold-start latency is covered by
the 900 ms race, so a slow function never blocks a tool call (`R4`).

## 6.4 Work units

| WU | Deliverable | Verify command | Expected output |
|---|---|---|---|
| `W1` | `api/_shared.ts` + `api/health.ts` | `npx vercel dev &` then `curl -s localhost:3000/api/health` | `{"ok":true,...,"catalog":{"products":60,"variants":200,"synthetic":true}}` |
| `W2` | `api/inventory/search.ts` + `api/inventory/filter.ts` | `curl -s -XPOST localhost:3000/api/inventory/search -d '{"query":"blue"}' -H 'content-type: application/json'` | `{"ok":true,"data":{"matches":[…` |
| `W3` | `api/shipping/quote.ts` | `curl -s -XPOST localhost:3000/api/shipping/quote -d '{"items":[{"sku":"<known>","qty":1}],"zone":4,"service":"ground"}' -H 'content-type: application/json'` | `"total_cents":` present, `"explain":[` non-empty |
| `W4` | `api/agent/plan.ts` (keyless path) | `curl -s -XPOST localhost:3000/api/agent/plan -d '{"goal":"hold low stock blue variants under $12 to zone 4"}' -H 'content-type: application/json'` | `"planner":"deterministic"` and ≥ 3 steps |
| `W5` | `src/engine/apiClient.ts` with local fallback | `npx vite-node -e "…apiClient.search({query:'blue'}) with the server DOWN"` | `{ ok: true, degraded: true }` |
| `W6` | tests under `tests/api/` including a 400 on bad input | `npm run test -- tests/api` | all pass |

---

# PART FIVE — required structure of the document you write

Write **one** file: `design_documents/design_plans/DP-SRV.md`.

```
# DP-SRV — Serverless API and typed client

## 1. Purpose & Scope
## 2. Requirements Traceability   (§3A block verbatim; DP-SRV owns MAN-05 jointly with DP-SHIP; enables FR-12, FR-19, NFR-01, NFR-06, NFR-08)
## 3. Architecture                (route map, the 900 ms race, where the key lives)
## 4. Interfaces                  (apiClient + the five routes with request/response shapes verbatim from §5C)
## 5. Algorithms                  (§6.2 items 1–5 as numbered steps, including the literal Gemini request body)
## 6. Configuration               (config keys read; env vars: GEMINI_API_KEY only, set via vercel env add)
## 7. Resiliency                  (§6.3 + which ladder rungs DP-SRV implements: 2, 3)
## 8. File Layout & Module Boundaries
## 9. Work Units                  (W1..W6 with Verify: lines and commit messages)
## 10. Testing Strategy
## 11. Dependencies & Dependents   (imports rows 1–16 and 19, 24, 26; consumed by DP-TOOLS, DP-AGENT, DP-UI)
## 12. Non-Goals Affirmation       (no business rules, no holds endpoint — holds are client-owned by design, no auth, no database, no chassis edits)
## Appendix A. Worked example      (a full POST /api/agent/plan round trip with the Gemini request, the raw response, and the validated ToolPlan)
```
