# PROMPT — Author design plan `DP-PITCH` (Deck, 3-minute script, demo capture, FAQ defence, submission text)
...
hackathon entry. Produce exactly one file:
    ...
    design_documents/design_plans/DP-PITCH.md
...
# PART FOUR — what `DP-PITCH` must own

`DP-PITCH` produces every artifact a judge reads or watches: the ≤3-minute video script and its
captured footage, the deck, the FAQ defence sheet, and `submission.md` with the four required
prompt answers. It invents no product claim that the running app does not support — every number
on a slide must be reproducible from the live URL or from `data/baseline.json`.

## 6.1 Required deliverables

| Deliverable | Path | Mandate |
|---|---|---|
| Timed script | `script.md` | `MAN-06` |
| Captured footage/screens | `assets/demodrive/<ts>/` | `MAN-06` |
| Deck | `deck/` (8–10 slides) + `docs/architecture.mmd` | supporting |
| FAQ defence | `docs/qa/` | supporting |
| Submission text | `submission.md` (four prompts + track line + no-bonus line) | `MAN-07`, `MAN-10` |

## 6.2 The video, specified to the second (`MAN-06` — under 3 minutes, first 3 min evaluated)

Total 2:45, leaving 15 s of headroom. Your plan must fix these beats, each with its on-screen
action, its spoken line, and its capture source:

| Time | On screen | Audio must say | Source |
|---|---|---|---|
| 0:00–0:25 | Maya's six tabs; the 25-minute batch | the problem, the persona, the 25-minute cost | slide + screen recording |
| 0:25–0:55 | The live URL opened in the **ChatGPT desktop in-app browser**; Tool Inspector panel listing the five tools with schemas | "the page registers five WebMCP tools with typed input schemas" — **this is the "how you used WebMCP" requirement, it must be audible** | live capture |
| 0:55–1:50 | The canonical goal typed; the co-execution timeline showing `search_inventory → filter_variants → calculate_shipping`, then the **confirmation dialog** for `hold_order` being clicked, then `confirm_fulfillment` | what the agent and the human each did, and why the confirmation step exists | live capture, demodrive fallback |
| 1:50–2:20 | DevTools → Application → WebMCP inspector, then a 5-second shot of `src/webmcp/register.ts` | "here is the imperative `registerTool` call — one call site, five tools, JSON Schema, annotations, abort handling" | screen capture |
| 2:20–2:45 | The savings meter and the delta 25 min → ~3 min; the track line | the impact claim, stated as measured on synthetic data | live capture |

Rules to state in the plan: audio is mandatory and must cover **what was built** and **how WebMCP
was implemented**; the upload must be **public on YouTube**; no copyrighted music; the first three
minutes are the only ones evaluated, so nothing important goes after 2:45.

## 6.3 Algorithms your plan MUST spell out step by step

1. **Script generation** — 1) `track status --json` first; any `stale` row blocks recording;
   2) `npx vite-node src/ideation/script/cli.ts generate --plan winning_project_plan.md --manifest
   assembly.manifest.json --config timing.yaml --out script.md`; 3) `timing.yaml` sets the five
   segment budgets from §6.2 (write the file in full); 4) hand-edit only the spoken lines, never the
   budgets; 5) assert the total ≤ 175 s.
2. **Capture** — 1) `npm run demodrive -- capture --script demo/click-script.json --data-source
   cache --cache-key opsflow-golden`; 2) this runs against the golden cache so a network failure
   cannot ruin a take; 3) captures land in `assets/demodrive/<ts>/`; 4) the live-browser segments
   (0:25–0:55 and 1:50–2:20) are recorded manually because they must show the real ChatGPT in-app
   browser and DevTools — state that these two are not automatable and must be filmed.
3. **Deck** — `npx vite-node src/ideation/deckgen/cli.ts populate …` then `… diagram --manifest
   assembly.manifest.json --out docs/architecture.mmd`. Fix the slide order: 1 title + track line,
   2 Maya and the 25-minute batch, 3 the five tools, 4 architecture diagram, 5 co-execution
   screenshot with the confirmation dialog, 6 WebMCP Leverage evidence (schemas, annotations,
   abort, limits, declarative form), 7 impact delta, 8 what is synthetic and what is real,
   9 roadmap, 10 links (live URL, repo, video).
4. **FAQ defence** — `npm run faqdef:generate …`, then add by hand the four questions this entry
   must never fumble, each with a written answer: (a) *why couldn't this be built before WebMCP?*
   (b) *what stops the agent doing something destructive?* (c) *what happens with a browser that
   has no WebMCP?* (d) *is the data real?* Write the model answers in the plan.
5. **`submission.md`** — `npx vite-node src/provenance/submit/cli.ts format --plan
   winning_project_plan.md --manifest assembly.manifest.json --disclosure disclosure.md --out
   submission.md`, then ensure it contains exactly these four `##` headings, in this order, and
   draft each in 120–200 words:
   ...
   Plus a one-line track statement (*WebMCP Challenge — Top 10*) and the literal sentence
   *"No bonus integration — not worth dilution."* (`MAN-10`), the live URL, the repo URL and the
   YouTube URL.

## 6.4 Resiliency requirements

Every demo segment has a captured fallback: if the live take fails on recording day, the
demodrive/golden-cache footage carries the same beats. If the video cannot be recorded at all, the
repo + description still satisfy the gate — Rules §4 allows judging on description, repo and video
alone — but a missing video is a `MAN-06` failure, so it is the highest-priority artifact after
the live URL.
...
|---|---|---|---|
| `W1` | `timing.yaml` + `script.md` | `npx vite-node src/ideation/script/cli.ts generate …` | total ≤ 175 s |
| `W2` | demodrive captures | `npm run demodrive -- capture …` | `assets/demodrive/<ts>/` non-empty |
| `W3` | `deck/` + `docs/architecture.mmd` | `npx vite-node src/ideation/deckgen/cli.ts diagram …` | valid mermaid, 10 slides present |
| `W4` | `docs/qa/` + the four hand-written answers | `rg -c "^### " docs/qa/*.md` | ≥ 12 questions |
| `W5` | `submission.md` | `rg -n "^## " submission.md` | exactly the four prompt headings |
| `W6` | freshness check before recording and before submitting | `track status --strict` | exit 0, no stale rows |

---

# PART FIVE — required structure of the document you write

Write **one** file: `design_documents/design_plans/DP-PITCH.md`.

```
# DP-PITCH — Deck, 3-minute script, demo capture, FAQ defence, submission text

## 1. Purpose & Scope
## 2. Requirements Traceability   (§3A block verbatim; DP-PITCH OWNS MAN-06, MAN-07, MAN-10)
## 3. Architecture                (which chassis ideation tool produces which artifact, and in what order)
## 4. Interfaces                  (every CLI command with its exact flags; the four submission headings verbatim)
## 5. Algorithms                  (§6.3 items 1–5, with timing.yaml and the four prompt answers drafted in full)
## 6. Configuration               (timing.yaml; deck slide order; demodrive cache key opsflow-golden)
## 7. Resiliency                  (§6.4; the capture fallback and the recording-day order of operations)
## 8. File Layout & Module Boundaries
## 9. Work Units                  (W1..W6 with Verify: lines and commit messages)
## 10. Testing Strategy           (script length assertion, four-heading assertion, track --strict)
## 11. Dependencies & Dependents   (consumes DP-SHIP's URL and disclosure, DP-SEED's capture data; owns no contract-table row)
## 12. Non-Goals Affirmation       (no claim the app cannot demonstrate, no second track, no bonus integration, no copyrighted media, no chassis edits)
## Appendix A. Worked example      (the full 2:45 script with every spoken line and every on-screen action)
```

## Finish

Write that one file and stop. Do not record anything and do not implement any code.
