# PROMPT — Author design plan `DP-DOM` (Domain Logic: catalog, filter, shipping, holds)
...
hackathon entry. Produce exactly one file:
    ...
    design_documents/design_plans/DP-DOM.md
...
# PART FOUR — what `DP-DOM` must own

`DP-DOM` is the only place business rules exist. Everything in it is **pure and isomorphic**:
no `fetch`, no React, no `document` — the single exception is `holdsStore`'s guarded
`localStorage` access, which must be wrapped in try/catch and work when storage throws or is
unavailable. `DP-DOM` never registers tools, never renders, never calls the network.

## 6.1 Required public surface (define these signatures exactly)

Contract-table rows 10–16 are yours.

```ts
// src/engine/domain/catalog.ts
export function loadCatalog(): Catalog;                       // memoized import of data/catalog.json
...
export function isLowStock(v: Variant): boolean;              // v.stock <= v.low_stock_threshold — the ONLY definition

// src/engine/domain/filter.ts
export function filterVariants(c: Catalog, i: FilterVariantsInput, fromSkus?: string[]): FilterVariantsOutput;

// src/engine/domain/shipping.ts
...
export function loadZones(): ZoneTable;                       // memoized import of data/zones.json
export function quoteShipping(c: Catalog, z: ZoneTable, i: CalculateShippingInput): ShippingQuote;

// src/engine/domain/holds.ts   (pure reducers — no storage)
...
export function newHoldId(rand?: () => number): string;       // "HOLD-" + 8 chars of [A-Z2-7]
export function newFulfillmentId(rand?: () => number): string;

// src/engine/domain/holdsStore.ts  (singleton, localStorage key "opsflow.holds.v1")
...
1. **`searchVariants`** — 1) lowercase and trim `query`, truncate to 200 chars; 2) split on
   whitespace into terms; 3) for every variant of every product build the haystack
   `[product.title, product.brand, product.category, variant.title, variant.sku,
   variant.options.size, variant.options.color].join(" ").toLowerCase()`; 4) a variant matches when
   **every** term is a substring of the haystack (AND semantics — state this explicitly); 5) if
   `inStockOnly === true`, drop variants with `stock === 0`; 6) sort by (a) `stock > 0` first,
   (b) ascending `price_cents`, (c) ascending `sku` — a total order, so results are deterministic;
   7) `total` = number of matches before truncation; 8) truncate to `limit ?? config.tools.default_limit`,
   capped at 100; 9) `truncated = total > matches.length`; 10) map each to `VariantMatch` with
   `low_stock: isLowStock(v)`; 11) `query_echo` is the truncated query.
2. **`filterVariants`** — 1) if `fromSkus` is a non-empty array, the candidate set is exactly those
   SKUs, in that order, and `from_result_set = true`; otherwise the candidate set is every variant
   and `from_result_set = false` (this is how constraint context is preserved across calls, `FR-03`);
   2) apply, in this order and skipping absent fields: `skuPrefix` (case-insensitive `startsWith`),
   `options.size` (exact, case-insensitive), `options.color` (exact, case-insensitive),
   `maxPriceCents` (`<=`), `minStock` (`>=`), `maxStock` (`<=`); 3) build `applied[]` with one
   human-readable sentence per filter actually applied, in the same order; 4) sort and truncate
   exactly as in `searchVariants` steps 6–9.
3. **`quoteShipping`** — 1) for each item, look up the variant; unknown SKU → push
   `{ sku, reason: "unknown sku" }` to `excluded` and skip; 2) `qty` outside 1..999 → exclude with
   reason `"invalid quantity"`; 3) `qty > variant.stock` → exclude with reason `"insufficient stock
   (have N)"`; 4) accumulate `total_weight_g += weight_g * qty` and `subtotal_cents += price_cents * qty`;
   5) `base_rate_cents = zone.base_cents + ceil(total_weight_g / 100) * zone.per_100g_cents`;
   6) multiply by `zone.service_multiplier[service]` and round **half up** to an integer;
   7) apply each surcharge from `ZoneTable.surcharges` whose condition matches — define each
   surcharge's condition literally in the plan (e.g. `OVERSIZE` when `total_weight_g > 10000`,
   `REMOTE` when `zone >= 4`, `EXPEDITED_FUEL` when `service !== "ground"`); 8) `total_cents =
   base_rate_cents + sum(surcharges)`; 9) build `explain[]`: one sentence per rule applied, at most
   12, in the order they were applied, each naming the number it produced; 10) never throw — an
   empty item list yields a zero quote with `explain: ["No items to rate."]`.
4. **`holdsStore.create`** — 1) validate `ttlMinutes` against `config.holds` min/max, else
   `INVALID_INPUT`; 2) validate every `lineItems[].sku` exists, else `NOT_FOUND`; 3) build the hold
   with `createHold`; 4) persist; 5) notify subscribers; 6) return
   `ok({ hold, requires_confirmation: true })`.
5. **`holdsStore.confirm`** — 1) `get(holdId)`; missing → `NOT_FOUND`; 2) `status === "confirmed"`
   → `CONFLICT`; `status === "released"` → `CONFLICT`; 3) `isExpired(hold, now)` → set status
   `"expired"`, persist, return `EXPIRED`; 4) otherwise build the `Fulfillment`, set status
   `"confirmed"`, persist, notify, return `ok({ fulfillment, hold })`. **Idempotence note:** a second
   confirm of the same hold returns `CONFLICT`, not a second fulfillment — state this.
6. **Persistence** — key `opsflow.holds.v1`, value `{ version: "1.0.0", holds: Hold[] }`. Reads and
   writes are wrapped in try/catch; on any failure the store keeps working from memory and logs once.
   Show the code.

## 6.3 Resiliency section requirements

No function in `DP-DOM` throws. Every failure returns a typed `ToolOutcome` via `makeToolError`.
All functions are deterministic given `(catalog, zones, now, rand)` — `now` and `rand` are always
injectable so tests are stable.

## 6.4 Work units

| WU | Deliverable | Verify command | Expected output |
|---|---|---|---|
| `W1` | `errors.ts` + `catalog.ts` | `npx vite-node -e "import('./src/engine/domain/catalog.ts').then(m=>console.log(m.searchVariants(m.loadCatalog(),{query:'blue'}).matches.length>0))"` | `true` |
| `W2` | `filter.ts` | `npx vite-node -e "import('./src/engine/domain/filter.ts').then(async m=>{const c=(await import('./src/engine/domain/catalog.ts')).loadCatalog();console.log(m.filterVariants(c,{maxPriceCents:1200,maxStock:5}).from_result_set)})"` | `false` |
| `W3` | `shipping.ts` + zone rules | `npx vite-node -e "…quoteShipping(c,z,{items:[{sku:'<known>',qty:2}],zone:4,service:'ground'}).explain.length"` | `>= 2` |
| `W4` | `holds.ts` pure reducers | `npx vite-node -e "…newHoldId(()=>0.5).startsWith('HOLD-')"` | `true` |
| `W5` | `holdsStore.ts` | `npx vite-node -e "…create then confirm then confirm again → codes"` | `ok` then `CONFLICT` |
| `W6` | tests under `tests/domain/` incl. a stock-boundary and an expired-hold case | `npm run test -- tests/domain` | all pass |

---

# PART FIVE — required structure of the document you write

Write **one** file: `design_documents/design_plans/DP-DOM.md`.

```
# DP-DOM — Domain Logic: catalog, filter, shipping, holds

## 1. Purpose & Scope
## 2. Requirements Traceability   (§3A block verbatim; DP-DOM enables FR-02..FR-06, NFR-02, NFR-04; owns no MAN-* outright)
## 3. Architecture                (pure isomorphic core; who imports it: DP-TOOLS, DP-SRV, DP-UI, DP-DEV)
## 4. Interfaces                  (every §6.1 signature verbatim; the §5A types it consumes)
## 5. Algorithms                  (every §6.2 item as numbered steps, including every surcharge condition)
## 6. Configuration               (which config/engine.json keys it reads; the ZoneTable file shape)
## 7. Resiliency                  (§6.3)
## 8. File Layout & Module Boundaries
## 9. Work Units                  (W1..W6 with Verify: lines and commit messages)
## 10. Testing Strategy           (boundary cases: zero stock, unknown sku, qty > stock, expired hold, empty query)
## 11. Dependencies & Dependents   (four boundary rules restated; consumers of rows 10–16)
## 12. Non-Goals Affirmation       (no fetch, no React, no registerTool, no chassis edits, no data generation — that is DP-SEED)
## Appendix A. Worked example      (one batch: search "blue" → filter maxPrice 1200 → quote zone 4 → hold 15 min → confirm, with the exact JSON at each step)
```
