# PROMPT — Author design plan `DP-CORE` (Engine Foundation: types, config, envelopes, resilience, context, usage)
...
hackathon entry. Produce exactly one file:
    ...
    design_documents/design_plans/DP-CORE.md
...
# PART FOUR — what `DP-CORE` must own

`DP-CORE` is the foundation every other plan imports. It owns the frozen types, the config
loader, the envelope publisher, the configured resilience wrapper plus golden cache, the agent
transcript buffer, and the cost-store writer. It contains **no** domain rules, **no** tool
registration, **no** HTTP handler, and **no** React.

## 6.1 Required public surface (define these signatures exactly)

Contract-table rows 1–9 are yours. Your plan must specify, verbatim:

```ts
// src/engine/types.ts — the full §5A block, unchanged, plus nothing else.

// src/engine/config.ts
export interface EngineConfig { /* exactly the §5E shape, every key typed */ }
export function loadConfig(): EngineConfig;          // memoized; throws only if config/engine.json is missing
export function configOverrides(): Partial<EngineConfig>;  // private helper — state it is private

// src/engine/envelopes.ts
export const publisher: CollectablePublisher;        // createPublisher("memory") — one instance per document
...
export function currentTraceId(): string | null;     // last id passed to emitToolEvent
...
export { isDegradedResult } from "src/resilience";   // re-export ONLY this predicate, for consumer convenience

// src/engine/context.ts
export function appendTranscript(role: "user" | "assistant" | "tool", text: string): void;
export function transcript(): Message[];             // already passed through chassis fit()
...
export function recordUsage(entry: UsageEntry): void;            // append to data/cost-store.json
export function usageTotals(): { prompt_tokens: number; completion_tokens: number; entries: number };
export function overBudget(): boolean;                            // true when totals exceed config/cost.json cap
```

Everything else in `src/engine/*.ts` is private; say so explicitly and state that no other
module may import it.

## 6.2 Algorithms your plan MUST spell out step by step

1. **`loadConfig()`** — 1) import `config/engine.json`; 2) for each of `RES_TIMEOUT_MS`,
   `RES_RETRIES`, `RES_FORCED_DEGRADED`, `OPSFLOW_PLANNER`, read `process.env` (node) or
   `import.meta.env` (browser) with a `typeof` guard so neither environment throws; 3) apply the
   override onto a deep copy; 4) memoize; 5) return. Write the guard code literally — the
   implementor must not have to work out how to read env in both runtimes.
2. **`emitToolEvent()`** — 1) build the envelope `{ step_id, status, payload, timestamp: new
   Date().toISOString(), sequence: <next>, trace_id, degraded }`; 2) call
   `publisher.publish({ stepId, status, payload, traceId, degraded })`; 3) never throw — wrap in
   try/catch and swallow, because a telemetry failure must never break a tool call; 4) store the
   trace id for `currentTraceId()`.
3. **`guarded()`** — 1) read `config.resilience`; 2) build the `ResilienceConfig` object literally
   (fill every field from config, including the `fallback_chain.order`); 3) call
   `withResilience(fn, cfg, { cache: goldenCache })`; 4) `await` it; 5) if
   `isDegradedResult(result)`, emit `session.degraded` with `{ reason, fallback_source }`;
   6) return the result **without throwing**. Show the exact code.
4. **Golden-cache key derivation** — `goldenCache.deriveKey({ provider: "opsflow", model:
   <tool or role name>, prompt: canonicalJson(input) })`, where `canonicalJson` sorts object keys
   recursively and drops `undefined`. Write `canonicalJson` in full pseudocode; it is the reason
   two runs with the same input hit the same cache entry.
5. **`recordUsage()`** — 1) read `data/cost-store.json` (create `{ version: "1.0.0", entries: [] }`
   if absent); 2) append the entry; 3) write back atomically (temp file + rename in node; no-op with
   a console warning in the browser); 4) the file must validate against the chassis contract
   `contracts/cost-store-snapshot.schema.json`. Token counts come from `count()` in `src/context` —
   the only tokenizer in the entry; never write your own.
6. **`appendTranscript()`/`transcript()`** — maintain a chassis `Buffer`, append each message, then
   call `fit(buffer, { max_tokens: config.context.max_tokens, reserve_output:
   config.context.reserve_output })` and keep the returned buffer. State what happens when the
   status is `over`: oldest non-system messages are dropped first (the chassis does this; do not
   re-implement it).

## 6.3 Resiliency section requirements

State literally: `emitToolEvent` never throws; `guarded` never throws; `loadConfig` throws only on
a missing config file and that failure is fatal at startup by design; `recordUsage` never throws;
`RES_FORCED_DEGRADED=1` forces rung 5 of the fallback ladder for the entire session.

## 6.4 Work units (design at least these; split further if any is too big for a weak model)

| WU | Deliverable | Verify command | Expected output |
|---|---|---|---|
| `W1` | `src/engine/types.ts` — §5A verbatim | `npx tsc --noEmit -p tsconfig.json` | no errors |
| `W2` | `config/engine.json` + `config/cost.json` + `src/engine/config.ts` | `npx vite-node -e "import('./src/engine/config.ts').then(m=>console.log(m.loadConfig().planner.model))"` | `gemini-2.5-flash` |
| `W3` | `src/engine/envelopes.ts` | `npx vite-node -e "import('./src/engine/envelopes.ts').then(async m=>{await m.emitToolEvent('tool.search_inventory','done',{ok:true});console.log(m.collectEnvelopes().length)})"` | `1` |
| `W4` | `src/engine/resilience.ts` | `RES_FORCED_DEGRADED=1 npx vite-node -e "import('./src/engine/resilience.ts').then(async m=>console.log(m.isDegradedResult(await m.guarded(async()=>1))))"` | `true` |
| `W5` | `src/engine/context.ts` | `npx vite-node -e "import('./src/engine/context.ts').then(m=>{m.appendTranscript('user','hi');console.log(m.transcript().length)})"` | `1` |
| `W6` | `src/engine/usage.ts` + first cost-store snapshot | `python3 -m src.cost.cli --store data/cost-store.json --budget config/cost.json` | a table with one row, exit 0 |

---

# PART FIVE — required structure of the document you write

Write **one** file: `design_documents/design_plans/DP-CORE.md`. Use exactly this skeleton.

```
# DP-CORE — Engine Foundation: types, config, envelopes, resilience, context, usage

## 1. Purpose & Scope
## 2. Requirements Traceability   (§3A MANDATORY COMPLIANCE block verbatim; DP-CORE owns none of MAN-01..MAN-10 outright — state that it inherits all of them and enables FR-14, FR-18, NFR-03, NFR-06, NFR-08)
## 3. Architecture                (where DP-CORE sits; who imports what; the §2.0 diagram reproduced)
## 4. Interfaces                  (every §6.1 signature verbatim + the full §5A types file + the §5E config file)
## 5. Algorithms                  (every §6.2 item as numbered steps; canonicalJson in full)
## 6. Configuration               (config/engine.json and config/cost.json in full, every key explained)
## 7. Resiliency                  (§6.3, plus the fallback ladder rungs DP-CORE implements)
## 8. File Layout & Module Boundaries (the §5D tree, plus .gitignore lines for .cache/, data/cost-store.json, .env*, .vercel/)
## 9. Work Units                  (W1..W6 with one Verify: line and one commit message each)
## 10. Testing Strategy           (unit tests under tests/engine/)
## 11. Dependencies & Dependents   (restate the four boundary rules from §3; list every consumer of rows 1–9)
## 12. Non-Goals Affirmation       (no domain rules, no registerTool, no HTTP, no React, no chassis edits)
## Appendix A. Worked example      (one tool call end to end: emitToolEvent started → guarded() degraded → session.degraded envelope → recordUsage row)
```

## Finish

Write that one file and stop. Do not implement any code. Do not write any other design plan.
